...
class SessionItem(ListItem):
    ...
    SessionItem:hover, SessionItem.active { background: $bg-hover; color: $text-bright; }
...
class Sidebar(Widget):
    ...
    DEFAULT_CSS = """
    Sidebar { width: 34; min-width: 30; background: $bg-panel; border-left: solid $border; layout: vertical; }
    ...
    .rail-tab:hover { background: $bg-hover; color: $text; }
    ...
    def __init__(self, on_llm_connect: Optional[Callable] = None, **kwargs):
        super().__init__(**kwargs)
        self.sessions: List[Dict] = []; self.tools: List[Dict] = []; self.files: List[Dict] = []
        self._on_llm_connect = on_llm_connect; self._llm_panel = None
    ...
    def on_mount(self) -> None:
        self._show("sessions")
        self._populate_sessions(); self._populate_tools(); self._populate_files()
    ...
    def _show(self, tab: str) -> None:
        ...
        try: self.query_one(f"#tab-{tab}", Button).add_class("active")
        except Exception: pass
        content = self.query_one("#sidebar-content")
        for child in content.children: child.display = False
        try: self.query_one(f"#sidebar-{tab}").display = True
        except Exception: pass
        if tab == "llm" and self._llm_panel is None: self._init_llm_panel()
    ...
    def _populate_sessions(self) -> None:
        try:
            view = self.query_one("#sidebar-sessions", ListView); view.clear()
            for s in self.sessions: view.append(SessionItem(s.get("id", ""), s.get("name", "Unnamed"), s.get("time", ""), s.get("active", False)))
        except Exception: pass
    def _populate_tools(self) -> None:
        try:
            view = self.query_one("#sidebar-tools", ListView); view.clear()
            for t in self.tools: view.append(ToolItem(t.get("name", ""), t.get("enabled", True)))
        except Exception: pass
    def _populate_files(self) -> None:
        try:
            view = self.query_one("#sidebar-files", ListView); view.clear()
            for f in self.files: view.append(ListItem(Label(f.get("name", ""))))
        except Exception: pass
    def set_sessions(self, sessions: List[Dict]) -> None: self.sessions = sessions; self._populate_sessions()
    def set_tools(self, tools: List[Dict]) -> None: self.tools = tools; self._populate_tools()
    def set_files(self, files: List[Dict]) -> None: self.files = files; self._populate_files()
    def get_llm_panel(self) -> Optional[LLMProviderPanel]:
        if self._llm_panel is None: self._init_llm_panel()
        return self._llm_panel
    def switch_to_llm(self) -> None: self._show("llm")
    def switch_to_tools(self) -> None: self._show("tools")
    def switch_to_sessions(self) -> None: self._show("sessions")
