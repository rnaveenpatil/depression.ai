...
class ModelItem(ListItem):
    ...
    ModelItem:hover, ModelItem:focus { background: $boost; }
    ModelItem.selected { background: $boost; border-left: tall $secondary; }
...
class ProviderHeader(ListItem):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(f"{info.get('name', self.provider)}", classes="provider-label")


class LLMProviderPanel(Widget):
    """Terminal-native model picker and LLM connection controls."""
    DEFAULT_CSS = """
    ...
    #llm-model-list { height: 1fr; min-height: 5; }
    #llm-config-area { height: 16; min-height: 16; padding: 1; border-top: solid $border; background: $panel; overflow-y: auto; }
    ...
    #llm-connect-btn:hover, #llm-connect-btn:focus { background: $boost; color: $text-bright; }
    ...
    def _status_text(self) -> str:
        ...
        if self.connection_status == "error":
            return "× connection failed · check key/url"
